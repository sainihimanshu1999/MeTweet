...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        Tweet.objects.create(content = 'hetooot', user = self.user)
        Tweet.objects.create(content = 'somebody come get her', user = self.userb)
        self.currentCount = Tweet.objects.all().count()
    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content = 'My  2nd tweet', user = self.user)
    ...
    def test_tweet_create_api_view(self):
        rdata = {'content': 'Hi i am Himanshu'}
        client = self.get_client()
        response = client.post('/api/tweets/create/', rdata)
        self.assertEqual(response.status_code, 201)
        data = response.json()
        new_tweet_id = data.get('id')
        self.assertEqual(self.currentCount+1, new_tweet_id)
